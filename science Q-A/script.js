document.addEventListener('DOMContentLoaded', function () {
  ...
  window.checkAnswer = function(correctAnswer) {
    ...
    const selectedInput = form.querySelector('input[name="science"]:checked');

    if (selectedInput) {
      const selectedValue = selectedInput.value;

      form.querySelectorAll('input[name="science"]').forEach(input => {
        input.disabled = true;

        if (input.value === correctAnswer) {
          const correctImage = input.previousElementSibling;
          correctImage.classList.remove('hidden');
          // input.classList.add('highlight-correct'); // Add a highlight class to the correct input
        }
      });
